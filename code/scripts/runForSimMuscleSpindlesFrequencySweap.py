...
def main():
	...
	if args.eesAmplitude[0]=="%": eesAmplitudeName=args.eesAmplitude[1:]
	else: eesAmplitudeName=args.eesAmplitude

	eesFrequencies = range(0,101,20)
	nProc = 4
	seed = "1"
	if args.phasicStim: emgVsKinMod = "kinemg"
	...
	for i,eesFrequency in enumerate(eesFrequencies):
		if not args.phasicStim:
			name = "Tonic_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_freq_"+str(eesFrequency)
			if args.species == "human":name += hp.get_dataset()
			resultFile = gt.find("*"+name+".p",pathToResults)
			if not resultFile:#'mpiexec','-np',str(nProc),
				program = ['python','./scripts/runForSimMuscleSpindles.py',\
					str(eesFrequency),args.eesAmplitude,args.species,inputFile,name,"--simTime",str(totSimTime),"--seed",seed,"--noPlot"]
		else:
			name = "Phasic_"+emgVsKinMod+"_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_freq_"+str(eesFrequency)
			if args.species == "human":name += hp.get_dataset()
			resultFile = gt.find("*"+name+"*.p",pathToResults)
			if not resultFile:
				program = ['mpiexec','-np',str(nProc),'python','./scripts/runForSimMuscleSpindlesStimModulation.py',\
					str(eesFrequency),args.eesAmplitude,args.species,emgVsKinMod,inputFile,name,"--simTime",str(totSimTime),"--seed",seed,"--noPlot"]
	...
	if not args.phasicStim: figName = time.strftime("/%Y_%m_%d_Tonic_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_firingRates.pdf")
	else: figName = time.strftime("/%Y_%m_%d_Phasic_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_firingRates.pdf")
	fig, ax = plt.subplots(2, 4,figsize=(16,9))
	cmap = plt.get_cmap('winter')
	colors = cmap(np.linspace(0.1,0.9,len(eesFrequencies)))

	for i,eesFrequency in enumerate(eesFrequencies):
		if not args.phasicStim: name = "Tonic_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_freq_"+str(eesFrequency)
		else: name = "Phasic_"+emgVsKinMod+"_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_freq_"+str(eesFrequency)
		if args.species == "human":name += hp.get_dataset()
...
# FIgure 5 plot 2 single gait cycles- afferent and efferents + mn phasicity score
	...
	if not args.phasicStim: figName = time.strftime("/%Y_%m_%d_Tonic_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_single_firingRates.pdf")
	else: figName = time.strftime("/%Y_%m_%d_Phasic_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_single_firingRates.pdf")
	...
	if not args.phasicStim: figName = time.strftime("/%Y_%m_%d_Tonic_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_afferentStats.pdf")
	else: figName = time.strftime("/%Y_%m_%d_Phasic_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_afferentStats.pdf")
	fig, ax = plt.subplots(2, 4,figsize=(16,9))
	cmap = plt.get_cmap('winter')
	colors = cmap(np.linspace(0.1,0.9,len(eesFrequencies)))
	bar_width = 5
