...
def main():
	...
	e.g: mpiexec -np 4 python scripts/runForSimMuscleSpindlesWithBaseline.py 40 240 rat frwSimRat.txt test
	"""
	...
	parser.add_argument("--tonicAfferents", help="tonic afferent firings", action="store_true")
	...
	if not args.tonicAfferents: afferentsInput = ldt.load_afferent_input(args.species,muscles)
	else: afferentsInput = ldt.load_afferent_input(args.species,muscles,"static")

	if args.species == "human": raise(Exception("TODO"))
	elif args.species == "rat": bs = BaselineActivity(pc,nn,rp.get_interneurons_baseline_fr())
