...
def main():
	...
	if args.species == "human":
		...
		if "SOL" in muscles: templateFile = "templateFrwSimHumanSOL.txt"
		else: templateFile = "templateFrwSimHuman.txt"
		w1 = 0.0210
		w2 = 0.0364
		w3 = 0.0165
		w4 = 0.0219
	...
	for i,eesFrequency in enumerate(eesFrequencies):
		...
		name = "HFLA_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_freq_"+str(eesFrequency)\
			+"_nPulses_"+str(nPulsesPerBurst)+"_burstFreq_"+str(burstsFrequency)
		...
		if not resultFile:
			program = ['mpiexec','-np',str(nProc),'python','./scripts/runForSimMuscleSpindles.py',\
				...
				"--burstingEes","--nPulsesPerBurst",str(nPulsesPerBurst),"--burstsFrequency",str(burstsFrequency)]
	...
	figName = time.strftime("/%Y_%m_%d_HFLA_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_firingRates.pdf")
...
# FIgure 5 plot 2 single gait cycles- afferent and efferents + mn phasicity score
	...
	figName = time.strftime("/%Y_%m_%d_HFLA_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_single_firingRates.pdf")
	...
	figName = time.strftime("/%Y_%m_%d_HFLA_FFS_species_"+toAddname+inputFileName+args.species+"_muscles_"+"".join(muscles)+"_delay_"+str(args.delay)+"_amp_"+str(eesAmplitudeName)+"_afferentStats.pdf")
	fig, ax = plt.subplots(2, 4,figsize=(16,9))
	cmap = plt.get_cmap('winter')
	colors = cmap(np.linspace(0.1,0.9,len(eesFrequencies)))
	bar_width = 5
