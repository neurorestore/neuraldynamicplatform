...
# lengthFile = "./generateForSimInputs/data/human/HuMoD-database/openSimOutputs/humanHumoD_B_ankleRangeWithKneeAtRest.txt" #considering halfROMknee
...
if subject == "B":
	gaitFile = "./generateForSimInputs/data/human/HuMoD-database/openSimOutputs/humanHumoD_B_gait"+speed+"_L.txt"
	...
	if speed == "1.1": motFile = "./generateForSimInputs/data/human/HuMoD-database/generatedFiles/Subject_B_speed_1.1_13-Apr-2017.mot"
	elif speed == "1.3": motFile = "./generateForSimInputs/data/human/HuMoD-database/generatedFiles/Subject_B_speed_1.3_10-Jul-2017.mot"

elif subject == "A":
	gaitFile = "./generateForSimInputs/data/human/HuMoD-database/openSimOutputs/humanHumoD_A_gait"+speed+"_LR.txt"
	fiberLengthGait = ldt.readOpensimData(gaitFile,['time','ext','flex'],["Coordinates(Deg.)",extMuscleOsim+"_"+side,flexMuscleOsim+"_"+side])
	fiberLengthGait['dt'] = (fiberLengthGait['time'][-1]-fiberLengthGait['time'].min())/fiberLengthGait['time'].size
	...
	if speed == "1.3": motFile = "./generateForSimInputs/data/human/HuMoD-database/generatedFiles/Subject_A_speed_1.3_31-Oct-2017.mot"
...
figName = time.strftime("/%Y_%m_%d_HumanAfferentsFirings_"+speed+subject+side+flexMuscleOut+extMuscleOut+".pdf")
...
figName = time.strftime("/%Y_%m_%d_HumanKin_"+speed+subject+side+flexMuscleOut+extMuscleOut+".pdf")
plt.savefig(pathToResults+figName, format="pdf",transparent=True)
